<template>
  <Teleport to="body">
    <Transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
      ...
      <div v-if="isOpen" class="fixed inset-0 z-[99999] overflow-y-auto" @click.self="cerrar">
        <!-- Backdrop -->
        ...
        <div class="flex min-h-full items-center justify-center p-4">
          <Transition enter-active-class="transition-all duration-300" enter-from-class="opacity-0 scale-95"
            ...
            <div v-if="isOpen"
              class="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl shadow-2xl z-[99999]" @click.stop>
              <!-- Header -->
              ...
              <div class="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
                <!-- Título -->
                <InputText v-model="form.titulo" name="" label="Título *" placeholder="Ingresa el título de la noticia"
                  :error="errores.titulo" />

                <!-- Contenido -->
                <InputTextarea v-model="form.contenido" name="contenido" label="Contenido *"
                  placeholder="Escribe el contenido del anuncio..." :rows="4" :error="errores.contenido" />

                <!-- Fechas -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <InputDate :modelValue="form.fecha_evento ?? ''" name="fecha_evento"
                    label="Fecha del evento (opcional)" :required="false"
                    @update:modelValue="form.fecha_evento = $event || null" />
                </div>

                <!-- Destacada -->
                <div class="flex items-center gap-3">
                  <button type="button" @click="form.destacada = !form.destacada" :class="form.destacada
                    ...
                    <span :class="form.destacada ? 'translate-x-5' : 'translate-x-0'"
                ...
                <div>
                  ...
                  <div v-if="previewUrl || form.imagen" class="mb-3 relative inline-block">
                    <img :src="previewUrl || form.imagen || ''" alt="Preview"
                  ...
                  <div class="space-y-2">
                    <div class="flex gap-2">
                      <input v-model="urlManual" type="url" placeholder="https://... (URL de imagen)"
              ...
              <div class="flex gap-3 p-6 border-t border-gray-200 dark:border-gray-700">
                <button type="button" @click="cerrar" :disabled="loading"
                ...
                <button type="button" @click="guardar" :disabled="loading"
...
const validar = (): boolean => {
  ...
  // Solo validamos lo que es estrictamente obligatorio
...
const guardar = () => {
  ...
  const formLimpio: INoticiaForm = {
    ...form.value,
    fecha_evento: form.value.fecha_evento?.trim() || null,
    fecha_publicacion: form.value.fecha_publicacion?.trim() || undefined,
  };

  emit('save', formLimpio);
};
