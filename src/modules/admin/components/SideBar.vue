<template>
  ...
  <transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
    ...
    <div v-if="isMobileOpen" @click="toggleMobileSidebar" class="fixed inset-0 bg-black/50 z-[99998] lg:hidden" />
  </transition>
  ...
  <aside :class="[
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-800">
      <router-link to="/" class="flex items-center gap-3 overflow-hidden"
        ...
        <div
          class="w-9 h-9 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
    ...
    <!-- Navigation -->
    <div
      class="flex-1 overflow-y-auto px-5 py-4 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-transparent">
      <nav class="space-y-6">
        <div v-for="(menuGroup, groupIndex) in menuGroups" :key="groupIndex">
          ...
          <ul class="space-y-1">
            <li v-for="(item, index) in menuGroup.items" :key="item.name">
              <!-- Button with submenu -->
              <button v-if="item.subItems" @click="toggleSubmenuLocal(groupIndex, index)" :class="[
                ...
                <span v-if="shouldShowText" class="font-medium text-sm flex-1 text-left">
                  {{ item.name }}
                </span>
                <ChevronDown v-if="shouldShowText" :class="[
              ...
              <router-link v-else-if="item.path" :to="item.path" @click="isMobileOpen && toggleMobileSidebar()" :class="[
                ...
                <span v-if="shouldShowText" class="font-medium text-sm">
                  {{ item.name }}
                </span>
              </router-link>
              ...
              <transition @enter="startTransition" @after-enter="endTransition" @before-leave="startTransition"
                ...
                <div v-show="isSubmenuOpen(groupIndex, index) && shouldShowText" class="overflow-hidden">
                  <ul class="space-y-1">
                    <li v-for="(item, index) in menuGroup.items" :key="item.name">

                      <button v-if="item.subItems" @click="toggleSubmenuLocal(groupIndex, index)" :class="[
                        'flex items-center gap-3 w-[calc(100%-1rem)] mx-2 px-3 py-2.5 rounded-xl transition-all duration-200 group',
                        'hover:bg-gray-100 dark:hover:bg-gray-800',
                        {
                          'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400': isSubmenuOpen(groupIndex, index),
                          'text-gray-700 dark:text-gray-300': !isSubmenuOpen(groupIndex, index),
                        ...
                        <ChevronDown v-if="shouldShowText"
                          :class="['w-4 h-4 transition-transform', { 'rotate-180': isSubmenuOpen(groupIndex, index) }]" />
                      </button>

                      <router-link v-else-if="item.path" :to="item.path" @click="isMobileOpen && toggleMobileSidebar()"
                        :class="[
                          // 'mx-2' crea el espacio para que el hover no toque los bordes
                          // 'w-[calc(100%-1rem)]' ajusta el ancho para compensar el margen
                          'mx-2 flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group',
                          'hover:bg-gray-100 dark:hover:bg-gray-800',
                          {
                            'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400': isActive(item.path),
                            'text-gray-700 dark:text-gray-300': !isActive(item.path),
                          },
                          shouldShowText ? 'justify-start' : 'justify-center',
                        ]">
                        ...
                        <span v-if="shouldShowText" class="font-medium text-sm whitespace-nowrap">
                          {{ item.name }}
                        </span>
                      </router-link>
    ...
    <!-- Footer -->
    <div v-if="shouldShowText" class="px-5 py-4 border-t border-gray-200 dark:border-gray-800">
...
const shouldShowText = computed(() => {
  return isExpanded.value || isMobileOpen.value;
});
