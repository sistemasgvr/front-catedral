...
CREATE TABLE public.authusuarios (
  ...
  rol text NOT NULL DEFAULT 'admin'::text CHECK (rol = ANY (ARRAY['admin'::text, 'operador'::text])),
...
-- Lineas vinculadas a la solicitud: intenciones en misa comunitaria, datos por niño (bautizo),
-- datos por pareja (matrimonio), etc. Una fila por cada unidad cobrada.
...
CREATE TABLE public.noticias (
  ...
  CONSTRAINT noticias_idusuariocreacion_fkey FOREIGN KEY (idusuariocreacion) REFERENCES public.authusuarios(idusuarios),
  CONSTRAINT noticias_idusuariomodificacion_fkey FOREIGN KEY (idusuariomodificacion) REFERENCES public.authusuarios(idusuarios)
);
...
CREATE TABLE public.solicitudes (
  ...
  fechacreacion timestamp with time zone DEFAULT now(),
  fechamodificacion timestamp with time zone,
  idestadoproceso integer DEFAULT 17,
  ...
  CONSTRAINT solicitudes_idestadoproceso_fkey FOREIGN KEY (idestadoproceso) REFERENCES public.opcioneslista(idopcionlista)
);
